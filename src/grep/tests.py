...
class Matcher:
    ...
    def start_compare(self, run_options: List[List[str]]) -> List[MatchResult]:
        ...
        for i, options in enumerate(run_options):
            ...
            if not result.is_success and self.stop_on_fail:
...
if __name__ == '__main__':
    ...
    matcher = Matcher(use_valgrind=use_valgrind, max_diff_count=None, stop_on_fail=config.stop_on_fail)
    try:
        match_results = matcher.start_compare(runnable_options)
